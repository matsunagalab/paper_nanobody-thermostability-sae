...
def train_run(
    ...
    save_logs_to_file=True,  # ローカルログファイル保存の有効/無効
):
    """
    ...
    Args:
        ...
        save_logs_to_file: Whether to save logs to local JSON file
    ...
    # Initialize logs list for local file saving
    logs_list = []
    ...
    # Training loop
    n_tokens_total = 0
    print(f"Starting training with {steps} steps, log_steps={log_steps}, save_logs_to_file={save_logs_to_file}")
    ...
    for step, act in enumerate(tqdm(data, total=steps)):
        ...
        # Logging metrics
        if log_steps is not None and (step % log_steps == 0 or step == steps - 1):
            ...
            if save_logs_to_file:
                ...
                if save_dir is not None:
                    log_file_path = os.path.join(save_dir, "training_logs.json")
                else:
                    log_file_path = "training_logs.json"
                print(f"Saving logs to {log_file_path}")
                # Save logs to file after each logging step
